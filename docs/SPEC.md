...
- `GET /locations/:id/map-thumb.png` — **public, no auth** (like `/health`): 480x240 PNG composed from OSM raster tiles (zoom 15) with a berry marker centered on the location's geom; in-memory cache (200 entries) + `Cache-Control: public, max-age=86400`; 404 when the location has no geom. When a location has a geom and `thumbnail_url` is null, serializers set `thumbnail_url` to this route. Query: `?maptype=roadmap|terrain|satellite|hybrid` (default roadmap, Google Static Maps style — ignored by the OSM fallback), `?refresh=1` (bypass + replace the in-memory cache entry), `?v=` (client style-version cache buster, ignored server-side). The cache key includes maptype and the active provider (Google key configured or not) so style/provider switches never serve stale bytes.
